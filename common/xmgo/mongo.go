...
func MustNewMongoCollection(config MongoCollectionConf, model any) *qmgo.QmgoClient {
	...
	qmgoClient, err := qmgo.Open(ctx, &qmgo.Config{
		...
		ConnectTimeoutMS: AnyPtr(int64(5000)),
		MaxPoolSize:      AnyPtr(config.MaxPoolSize),
		MinPoolSize:      AnyPtr(config.MinPoolSize),
		SocketTimeoutMS:  AnyPtr(config.SocketTimeoutMS),
